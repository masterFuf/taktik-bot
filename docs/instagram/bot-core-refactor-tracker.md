...
<div class="doc-hero">
  <div class="doc-kicker">Suivi transverse</div>
  <p>Cette page suit le refactor de <code>bot/taktik/core</code> pour Instagram et TikTok. Elle existe pour eviter de cacher l'avancement du Bot core dans une seule doc plateforme.</p>
  <p>Les audits Instagram et TikTok gardent leurs constats propres. Ici, on suit uniquement le chantier commun : taxonomie <code>core</code>, boundaries shared/runtime/database, selectors scopes, compat et lots restants.</p>
</div>

## Pourquoi cette page existe

- La doc consolidee expose des docs dediees Instagram et TikTok.
...
- [x] Le workflow `tiktok publish` n'instancie plus le notifier bridge dans `core`; le bridge route par la cle manifest `tiktok_publish_bridge` (`bridges.tiktok.publish.publish`) le lui injecte maintenant.
...
- la racine `ui/selectors/` n'est plus une zone de depot ;
- les owners reels sont les sous-packages scopes ;
- les agregateurs publics restent des facades de compatibilite jusqu'a validation runtime plus large.

## Lire ensuite

- Pour le contexte Instagram et les risques historiques : [Audit qualite et refactor Instagram](quality-audit.md)
- Pour le plan de remediation Instagram : [Plan de colmatage Instagram](audit-remediation-plan.md)
- Pour le contexte TikTok equivalent : ouvrir la meme page cote TikTok depuis le menu local TikTok.
