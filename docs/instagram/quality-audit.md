...
| P0 Ownership SQLite/sync | 74% | Beaucoup de handlers passent par repositories/services et `database:contracts` protege les boundaries. L'inventaire des ecritures Electron vs Python est documente par scan (matrice P0-2 de `audit-remediation-plan.md`) et la convention d'ownership est tranchee : **« Python ecrit les faits, Electron enrichit »** (Python owner des sessions/interactions/stats/sync/colonnes factuelles ; Electron owner IA/media/geo/scheduler/taxonomy/desktop, lecture des faits). Reste l'application code progressive (reduire les ecritures factuelles Electron sur les tables partagees) et le diagnostic sync complet. |
