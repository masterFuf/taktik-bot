...
class InteractionEngineMixin:
    ...
    def _perform_interactions_on_profile(
        ...
        try:
            ...
            if story_phase == 'start':
                ...
                if result.get('stories_liked') and self._stop_if_action_blocked(username, 'story like'):
                    return result
            ...
            if should_like or should_comment:
                ...
                if self._stop_if_action_blocked(username, 'like/comment'):
                    return result
    ...
    def _stop_if_action_blocked(self, username: str, action: str) -> bool:
        ...
        trying its next action. One dump per write phase. Never raises: a check that cannot read
        the screen is not a block.
        """
    ...
    def _do_follow(self, username, plan, profile_data, result) -> bool:
        ...
        via the result['follows'] guard. True when the follow button was tapped, landed or not: a
        refused follow is exactly the one to look for a block after."""
