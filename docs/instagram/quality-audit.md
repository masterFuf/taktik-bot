...
| P1 SQL direct handlers | 81% | `targetSearch`, scraping, DB commun, scheduler, analytics/profiling, sessions/interactions/stats, accounts/profiles/following, storage media cleanup, configuration locale device/network et plusieurs routes TikTok ont ete nettoyes. Cote Electron, Cold DM ne remappe plus les profils scraping dans le handler, Target Search n'ecrit plus l'export CSV depuis l'IPC, `storage:*` delegue usage disque + cleanup DB a un service owner, `device-groups`/`network-*` passent par des services app, `analytics.ts` delegue a `AnalyticsProfilingService`, `sessions.ts` delegue a `SessionDatabaseService` et `accounts.ts` delegue a `InstagramAccountDatabaseService`. Reste des zones ponctuelles comme AI media/diagnostics et exceptions documentees. |
...
Estimation globale actuelle : environ **96%** du chantier front/Electron
