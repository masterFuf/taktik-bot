...
def test_a_search_run_reports_the_motive_of_its_last_query(monkeypatch):
    from bridges.tiktok.workflows.automation.runtime import search_query

    for name in ("send_message", "send_status", "set_workflow", "setup_search_workflow_callbacks"):
        monkeypatch.setattr(search_query, name, lambda *_a, **_k: None)
    monkeypatch.setattr(search_query, "build_search_config", lambda *_a, **_k: object())

    motives = iter(["feed_stuck", ""])
    ...
    class _Workflow:
        ...
        def run(self):
            return VideoWorkflowStats(completion_reason=next(motives))

    total = VideoWorkflowStats()
    manager = SimpleNamespace(device_manager=SimpleNamespace(device=object()))

    def run(index):
        search_query.run_search_query(
            _Workflow, object, manager, {}, ["a", "b"], index, ["a", "b"][index], "hashtag",
            5, 10, 10, total, set(),
        )

    run(0)
    assert total.completion_reason == "feed_stuck"
    run(1)   # the next query moved: the run did not end on a frozen feed
    assert total.completion_reason == ""
