...
2. Lot 2 - ownership DB Instagram : sous-lots faits pour `processed / filtered / skip`, `processed_hashtag_posts` et `unfollow sync`. Le SQL `following_sync` / `followers_sync` vit maintenant dans `repositories/instagram/social_graph/`, la famille `unfollow` appelle deja `InstagramFollowGraphService` directement, la famille `hashtag` appelle deja `InstagramHashtagPostService` directement, et le runtime Instagram (business + UI extractors) appelle maintenant `InstagramWorkflowStateService` directement pour `processed / filtered / skip`. `DatabaseHelpers` ne sert plus qu'en shim de compatibilite package-level. Prochain pas naturel : auditer si cette API de compat est encore necessaire hors repo avant suppression ou re-export plus fin.
