...
- Dans `taktik/core/agent/**`, utiliser les owners internes directs : `kernel/contracts.py` pour les dataclasses de plan/event, `kernel/ports.py` pour les interfaces injectees, `kernel/` pour contexte/registry/executor/runtime, `io/` pour manifest/plan/events, `decision/` pour la decision IA, `scenarios/` pour les autopilots legacy. La racine du package doit rester une facade publique via `__init__.py`, pas une liste de modules plats.
