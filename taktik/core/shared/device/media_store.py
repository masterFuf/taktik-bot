...
def delete_pushed_media(
    ...
    """Delete one exact bot-owned gallery copy and its MediaStore rows.

    Refuse paths outside ``remote_dir`` and names without the timestamp format
    generated by :func:`push_media`. This function never touches the local source.
    """
    expected_dir = remote_dir.rstrip("/")
    if os.path.dirname(remote_path) != expected_dir:
        return False
    if parse_pushed_timestamp(os.path.basename(remote_path), file_prefix) is None:
        return False
    ...
    if log:
        log('info', f'[media_store] deleted temporary media: {os.path.basename(remote_path)}')
    return True
