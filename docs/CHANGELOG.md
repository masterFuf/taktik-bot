...
- Instagram desktop automation bridge cleanup started: debug commands now live under `bridges/instagram/diagnostics/debug.py`, CLI/stdin config loading under `bridges/instagram/automation/input.py`, and optional media capture lifecycle under `bridges/instagram/automation/media_capture.py`, leaving `automation/desktop.py` focused on session orchestration.
- Instagram desktop workflow execution now lives in `bridges/instagram/automation/workflow_runner.py`, with AI service setup in `automation/ai_runtime.py`; `automation/desktop.py` delegates workflow preparation, session config events, AI hook installation and final stats emission to the runner.
- Instagram desktop bridge runtime lifecycle now lives in `bridges/instagram/automation/runtime.py`: SQLite setup, device connection, app launch, optional network reset and app cleanup are no longer embedded in `automation/desktop.py`.
