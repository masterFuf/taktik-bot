...
class _Sheet:
    """A comment sheet whose openness, rows and composer are each set independently."""

    def __init__(self, *, is_open=True, authors=(), bodies=(), replies=0, composer=""):
    ...
    def xpath(self, selector):
        ...
        if "focusable" in selector:
            return _Result([_Node(b) for b in self.bodies])
...
def test_a_comment_keeps_its_own_body():
    actions = _actions(_Sheet(authors=["marie", "paul"], bodies=["bonjour", "salut"]))
    assert actions.read_comments() == [
...
def test_a_missing_like_count_is_none_not_zero():
    ...
    comments = _actions(_Sheet(authors=["marie"], bodies=["bonjour"])).read_comments()
