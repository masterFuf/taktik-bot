...
- Dans `taktik/core/device/**`, la compat legacy doit vivre sous `device/compat/**`. Le module `device.py` est seulement un shim pour les anciens imports directs `taktik.core.device.device`.
...
- Si un module top-level `taktik/core/<family>` est en pratique specifique a une seule plateforme, l'implementation doit vivre sous `taktik/core/social_media/<platform>/...` et `taktik/core/<family>` ne doit rester que comme facade/shim de compatibilite temporaire. `taktik/core/recorder/**` suit maintenant cette regle pour le recorder Instagram.
- Pour les facades legacy mono-plateforme comme `taktik/core/recorder` ou `taktik/core/media`, preferer une facade package-level `__init__.py` plutot qu'un module racine additionnel si aucun consommateur runtime interne n'utilise l'ancien sous-module.
- Un module sous `taktik/core/**` ne doit pas importer directement `bridges.common.*` pour obtenir son IPC, son AI service ou un autre adaptateur desktop. Le bridge ou le workflow appelant doit injecter un notifier/service optionnel depuis l'exterieur. `core/email/gmail/gmail_workflow.py` suit maintenant cette regle via un notifier injecte.
