...
| B3 | A faire | Extraire les implementations Instagram engagement (`dm`, `cold_dm`, `smart_comment`) derriere des entrypoints publics minces ou deplacer seulement les helpers internes sans casser les chemins dev. | Tests/imports bridge + manifest + audit stdout. |
| B4 | A faire | Continuer `bridges/common` par capacite (`input/keyboard`, `persistence/database`, `runtime/ipc`) si les imports peuvent etre migres sans shim. | Import graph + checks bridge. |
| B5 | A faire | Examiner YouTube/Gmail/Threads pour ne pas sur-organiser les petits dossiers. | Manifest + compileall. |
