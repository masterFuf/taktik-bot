...
- Gmail account workflows now expose injectable Agent handlers for `gmail.account.login`, `gmail.account.read_otp`, and `gmail.account.scan_accounts`; account DB persistence is optional and injected, while Gmail logout remains bridge-owned until a real core workflow exists.
