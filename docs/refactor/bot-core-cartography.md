...
| `compat/` | Compatibilite version/selectors | Routing de selectors par version, tracing compat et overrides YAML. | `clone`, bridges de compat, bridge Instagram desktop. | Aucun import top-level sortant releve. | Frontiere utile mais fragile si on y depose du debug ou du runtime generique. Les owners selectors/versioning etaient encore a plat au niveau racine. | Garder comme zone de compatibilite explicite, avec `compat/selectors/**` comme owner interne du framework selectors/versioning ; laisser les fichiers top-level seulement en shims de compatibilite tant que necessaire. |
...
2. Lot 2 - ownership DB Instagram : sous-lots faits pour `processed / filtered / skip`, `processed_hashtag_posts` et `unfollow sync`. Le SQL `following_sync` / `followers_sync` vit maintenant dans `repositories/instagram/social_graph/`, la famille `unfollow` appelle deja `InstagramFollowGraphService` directement, la famille `hashtag` appelle deja `InstagramHashtagPostService` directement, le runtime Instagram (business + UI extractors) appelle maintenant `InstagramWorkflowStateService` directement pour `processed / filtered / skip`, `workflows/post_scraping/post_persistence.py` ne porte plus de SQL inline sur `instagram_profiles`, et le scraping Instagram partage maintenant une dependency `local_db` explicite entre `scraping_workflow`, `persistence`, `list_scraping` et `deep_qualify` au lieu de multiplier les `get_local_database()` caches dans chaque mixin. `DatabaseHelpers` ne sert plus qu'en shim de compatibilite package-level. Prochain pas naturel : auditer si cette API de compat est encore necessaire hors repo avant suppression ou re-export plus fin.
...
4. Lot 4 - clarifier `clone/compat` : `clone/**` reste l'owner du runtime clone-aware (package actif, proxy device, patch package-name), tandis que `compat/**` reste l'owner de la compatibilite selectors/versioning. Premier sous-lot fait le 2026-05-31 : le framework selectors/versioning de `compat` vit maintenant sous `compat/selectors/**`, les bridges internes pointent vers ces owners directs, et les anciens modules top-level `compat/selector_registry.py`, `compat/selector_tracer.py`, `compat/setup.py` restent comme shims de compatibilite.
