...
- `social_media/tiktok/actions/business/workflows/dm/outreach.py` porte maintenant la logique metier cold outreach ; le bridge route par la cle manifest `dm_outreach_bridge` (`bridges.tiktok.engagement.dm_outreach`) injecte seulement le notifier stdout JSON et la dedup SQLite. Le handler Agent outreach pourra etre ajoute dans un lot separe avec ces memes injections.
...
- `app/email/gmail/workflows/agent_handler.py` expose `gmail.account.login`, `gmail.account.logout`, `gmail.account.read_otp` et `gmail.account.scan_accounts`, avec persistence account optionnelle injectee. Le logout Gmail ouvre les settings Android pour confirmation manuelle, comme le bridge historique, mais la logique vit maintenant dans le workflow core.
- `social_media/youtube/workflows/account/` porte maintenant le workflow account YouTube extrait du bridge et expose `youtube.account.login` / `youtube.account.logout` comme handlers injectables. La connexion device, le bootstrap DB, le `force_stop` et les messages stdout JSON restent dans le bridge route par la cle manifest `youtube_account_bridge` (`bridges.youtube.account.account`).
...
- Instagram engagement/agent : les cles manifest `dm_bridge`, `cold_dm_bridge`, `smart_comment_bridge` et `taktik_agent_bridge` routent vers des modules scopes, mais n'ont pas encore toutes une frontiere core homogene par workflow.
- Threads automation : les workflows search/feed acceptent maintenant un `startup` injecte et `social_media/threads/workflows/agent_handler.py` enregistre `threads.automation.follow/target/feed` sans ouvrir de connexion device. Le bridge garde `threads_startup()` par defaut.
