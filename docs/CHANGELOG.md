...
### Changed
- Aligned the remaining Bot core/transverse source docs with the verified consolidated mirrors: `annexes/contributing.md`, `core/agent-ai.md`, `core/config-clone-security.md`, `core/media-recorder.md` and `core/taktik-agent-autonomous-orchestration.md` now use the current `core/app/**` and Instagram media/recorder paths, clarify the OpenRouter/fal.ai split, mark Taktik Agent orchestration as partially implemented, and replace stale flat bridge or quota wording.
