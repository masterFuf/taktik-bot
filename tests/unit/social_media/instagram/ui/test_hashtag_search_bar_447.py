...
follows that screen (the explore grid under the search bar); texts are Instagram's own.
"""
...
def test_the_search_bar_is_found_whatever_its_class(bar_class):
    assert _first_match(_search_screen(bar_class)) == f"{IG}/action_bar_search_edit_text"


def test_the_first_entry_is_the_bar_itself_not_the_search_tab():
