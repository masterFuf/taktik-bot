"""Instagram profile data model compatibility objects."""
            ...
            if isinstance(instance, InstagramProfile):
                try:
                    ...
                    if result and "profile_id" in result:
                        ...
                        self.logger.info(
                            "Profile %s saved via API with ID: %s",
                            instance.username,
                            result["profile_id"],
                        )
                except Exception as exc:
                    self.logger.error("Error saving via API: %s", exc)
...
class DatabaseQuery:
    ...
    def first(self):
        ...
        for filter_value in self.filters:
            if isinstance(filter_value, tuple) and filter_value[0] == "username":
                username = filter_value[1]
                break
        ...
        if username and self.model_class == InstagramProfile:
            try:
                ...
                self.session.logger.error("Error retrieving via API: %s", exc)
        return None
