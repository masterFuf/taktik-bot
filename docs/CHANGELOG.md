...
### Changed
- Clarified the Instagram upload workflow doc after checking the live router: `workflows/upload-content.md` no longer describes `UploadPost.tsx` as active. It is now documented as a legacy non-routed page, while `UploadPostAIPage` remains the current routed UI for `post`/`reel`/`story`.
- Corrected the Instagram publish current-state docs after checking the live renderer routes: `desktop/platform-features.md` no longer presents `UploadPost.tsx` as an active routed surface. The current route uses `UploadPostAIPage` for `post`/`reel`/`story`, while `UploadPost.tsx` stays legacy/non-routed and `UploadReel.tsx` remains a disabled placeholder.
- Aligned the remaining Instagram source docs with the verified consolidated mirrors: the Instagram hub/runtime/data/audit/cartography/publish/scraping/selectors/workflow pages now use launcher bridge names, current `services/app/scheduler` and repository paths, portable temp-path wording, the real post-Discovery scraping/qualification map, and the current state of the publish Electron-to-Bot migration instead of stale flat bridge filenames, Discovery wording or local-machine paths.
- Aligned the remaining Bot core/transverse source docs with the verified consolidated mirrors: `annexes/contributing.md`, `core/agent-ai.md`, `core/config-clone-security.md`, `core/media-recorder.md` and `core/taktik-agent-autonomous-orchestration.md` now match the current `core/app/**`, `clone/**` and Instagram media/recorder owners, clarify the OpenRouter/fal.ai split, keep the Taktik Agent page explicit about its partially implemented premium target, and retain quota wording only where the Python runtime still uses real local quotas.
- Aligned the remaining transverse workflow source docs with the verified consolidated mirrors: `workflows/dm-cold-dm.md`, `flow-diagrams.md`, `scheduler-next-steps.md`, `sessions.md`, `smart-comment.md`, `tiktok.md` and `upload-content.md` now use launcher bridge names, current preload/repository/service paths, the real scheduler node support/stop routes, and the post-Discovery workflow ownership instead of stale flat bridge filenames or placeholder scheduler wording.
- Aligned the Bot transverse/audit source docs with the verified consolidated mirrors: `database/overview.md`, `web/overview.md`, `modules/instagram/{documentation-audit,tests}.md` and `tiktok/{bot-core-refactor-tracker,debug-compat,quality-audit}.md` now use consolidated-doc references, post-Discovery wording, portable temp/repo paths and current bridge/module names instead of stale GitBook, Discovery or flat-bridge wording.
- Aligned the remaining Bot TikTok tracker/audit source docs with the verified consolidated mirrors: `tiktok/bot-core-refactor-tracker.md` and `tiktok/quality-audit.md` now use the current bridge/module naming instead of stale flat bridge filenames presented as active routes.
- Aligned the Bot compat/transverse source docs with the verified consolidated mirrors: `compat/bridge-tooling.md`, `compat/testing-framework.md`, `compat/versioned-selectors.md`, `security/battery-spoofing.md` and `web/overview.md` now use the current diagnostics entrypoints/runtime tree, the Cartography Lab naming, the current preload path `preload/devices/adb.ts`, and `taktik-docs` as the canonical transverse technical documentation instead of stale flat compat bridges or the old Bot-docs canonical wording.
- Aligned the Bot TikTok source docs with the verified consolidated mirrors: `tiktok/README.md`, `architecture.md`, `bridges.md`, `workflows.md`, `shared-modules.md`, `bot-runtime.md`, `electron-runtime.md` and `data-contracts.md` now document launcher bridge names, current dispatcher/module paths, the `login` `not_implemented` limitation, and the current `services/app/scheduler` / shared Electron paths instead of stale flat bridge files or old service locations.
- Aligned the Bot navigation/refactor source docs with the verified consolidated mirrors: `guidebook-map.md` and the `refactor/*` workorders now reference the consolidated private docs, current launcher-based bridge names, the real Electron workflow-registry paths and the fact that some historical audits/scripts are legacy or need re-validation before being treated as reliable guards.
- Aligned the Bot module/transverse source docs with the verified consolidated mirrors: the Instagram/TikTok/Gmail/Threads/YouTube overview pages, `modules/instagram/business-actions.md`, `modules/instagram/scraping-workflows.md`, `scope.md`, `glossary.md` and `social-docs.md` now use launcher bridge names, post-Discovery workflow wording, current module paths and the private consolidated-doc framing instead of stale flat bridge files or removed Discovery workflows.
- Aligned the Bot getting-started and documentation-governance source docs with the verified consolidated mirrors: installation/configuration/quick-start pages now use portable repo paths and current launcher bridge names, and the roadmap/strategy pages now reference the private consolidated documentation, post-Discovery architecture and the current `services/app/scheduler` / SQLite reference wording.
- Aligned the remaining Bot bridge source docs with the verified consolidated mirrors: `bridges/common-services.md`, `bridges/{instagram,tiktok,threads,gmail,youtube}.md` now reflect the manifest/launcher module paths, current runtime responsibilities and the post-Discovery bridge surface instead of legacy flat bridge files and outdated workflow inventories.
- Aligned the remaining Bot architecture source docs with the verified consolidated mirrors: `clone-package-support.md`, `overview.md`, `sequence-diagrams.md`, `sync-cross-device.md` and `bridges/architecture.md` now use launcher bridge names, current sync/preload paths, the post-Discovery architecture wording and the active bridge registry instead of stale flat bridge files and old front sync paths.
- Aligned the Bot architecture/bridge source docs with the verified consolidated mirrors: `architecture/application-map.md`, `architecture/layers.md`, `bridges/launcher.md` and `bridges/ipc-protocol.md` now describe launcher-based bridge names, the post-Discovery workflow map, the current TikTok/compat bridge registry and the launcher-centric Electron contract instead of stale flat `*_bridge.py` paths.
- Aligned the scheduler taxonomy source docs with the current private mirror: `desktop/scheduler-taxonomy-v2.md` now explicitly treats `discovery` as a scheduler/taxonomy intent rather than the removed standalone Discovery workflow, and the taxonomy pages now link to the local `scheduler-taxonomy-pipeline.md` / `scheduler-taxonomy-prompts.md` docs instead of stale `front/docs/Scheduler/*` paths.
