...
2. Lot 2 - ownership DB Instagram : sous-lots faits pour `processed / filtered / skip`, `processed_hashtag_posts` et `unfollow sync`. Le SQL `following_sync` / `followers_sync` vit maintenant dans `repositories/instagram/social_graph/`, la famille `unfollow` appelle deja `InstagramFollowGraphService` directement, la famille `hashtag` appelle deja `InstagramHashtagPostService` directement, et plusieurs workflows (`notifications`, `followers`, `likers`, `feed`) passent deja par `InstagramWorkflowStateService` sans shim. Prochain pas naturel : reduire `instagram_follow_graph.py` puis supprimer `database_helpers.py` quand les derniers call sites legacy de `profile_processing` / `interaction_engine` auront disparu.
