...
| P0 Ownership SQLite/sync | 73% | Beaucoup de handlers passent par repositories/services et `database:contracts` protege les boundaries. L'inventaire des ecritures Electron vs Python est maintenant documente par scan dans `audit-remediation-plan.md` (matrice P0-2) : dual-write confirme sur `sessions`, `scraping_sessions`, `instagram_profiles`, `profile_stats_history`, `daily_stats`, `interaction_history`, `instagram_accounts`. Reste a contractualiser l'owner cible de ces tables dual-write et a brancher le diagnostic sync complet. |
