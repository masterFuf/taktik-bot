...
| P0 Ownership SQLite/sync | 77% | Beaucoup de handlers passent par repositories/services et `database:contracts` protege les boundaries. L'inventaire des ecritures Electron vs Python est documente par scan (matrice P0-2 de `audit-remediation-plan.md`) et la convention d'ownership est tranchee : **« Python ecrit les faits, Electron enrichit »** (Python owner des sessions/interactions/stats/sync/colonnes factuelles ; Electron owner IA/media/geo/scheduler/taxonomy/desktop, lecture des faits). `npm run database:contracts` enforce deja la convention pour les tables purement Python (`following_sync`, `followers_sync`, `sent_dms`) : aucune ecriture repository Electron permise. Le contrat de champs colonne-par-colonne des tables partagees (`instagram_profiles`, `instagram_accounts`) est documente dans `audit-remediation-plan.md` (releve par scan des INSERT/UPDATE reels). Regle d'enrichissement IA tranchee : Electron ecrit ses valeurs derivees dans des colonnes paralleles `ai_<champ>` sans ecraser le champ factuel Python (les `ai_*` existants sont conformes ; `account_based_in` geo reste a migrer vers `ai_account_based_in`). Reste l'application code progressive (migration `ai_account_based_in`, `ProfileRepository.update()` en lecture sur le factuel, garde colonne) et le diagnostic sync complet. |
| P1 SQL direct handlers | 92% | Plus aucun handler Electron (`electron/handlers/**`) n'importe `databaseService` ni n'appelle `db.prepare`/`.prepare(` : scan `rg` vide. `targetSearch`, scraping, DB commun, scheduler/content-planner, analytics/profiling, sessions/interactions/stats, accounts/profiles/following, Smart Comment DB, Gmail DB, relationship graph DB, TikTok DB, database explorer, DB export/tutorial, startup cleanup DB, target search DB owner, ADB network snapshots, AI profile-classify account read, storage media cleanup, configuration locale device/network et plusieurs routes TikTok ont ete nettoyes. Cote Electron, Cold DM ne remappe plus les profils scraping dans le handler, Target Search n'ecrit plus l'export CSV depuis l'IPC, `storage:*` delegue usage disque + cleanup DB a un service owner, `device-groups`/`network-*` passent par des services app, `analytics.ts` delegue a `AnalyticsProfilingService`, `sessions.ts` delegue a `SessionDatabaseService`, `accounts.ts` delegue a `InstagramAccountDatabaseService`, `smart-comment.ts` delegue a `SmartCommentDatabaseService`, `gmail.ts` delegue a `GmailAccountDatabaseService`, `graph.ts` delegue a `RelationshipGraphDatabaseService`, `tiktok.ts` delegue a `TikTokDatabaseService`, `explorer.ts` delegue a `DatabaseExplorerService`, `export.ts` delegue a `DatabaseExportService`/`TutorialProgressService` et le registrar DB commun delegue le cleanup de boot a `DatabaseStartupCleanupService`. Reste des zones ponctuelles comme AI media/diagnostics et exceptions documentees. |
...
Estimation globale actuelle : environ **99%** du chantier front/Electron
