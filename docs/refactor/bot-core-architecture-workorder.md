...
- un sous-lot suivant sort le SQL `sent_dms` de `bridges/common/database.py` vers `taktik/core/database/repositories/messaging/SentDMRepository`, en gardant le bridge comme facade de compatibilite et sans modifier le schema partage Electron/Python.
- un sous-lot suivant clarifie `taktik/core/database/models` en remplacant le doublon historique `models/models.py` par l'owner explicite `models/instagram_profile.py`, tout en gardant l'import public `taktik.core.database.models`.
...
- un sous-lot suivant migre `taktik/core/email` vers `taktik/core/app/email` sans shim racine ; Gmail, YouTube et TikTok signup importent l'owner app direct.
...
- un sous-lot suivant clarifie les primitives ADB partagees : `run_adb_shell` vit maintenant sous `shared/device/adb.py`, les consumers device/runtime importent cet owner direct, et `shared/input/taktik_keyboard.py` garde seulement un re-export de compatibilite.
- un sous-lot suivant ajoute aussi `run_adb_shell_process()` sous `shared/device/adb.py` pour les appels qui ont besoin du `returncode`, et migre les helpers bridge `app_control` / `app_inspection` dessus.
- un sous-lot suivant etend `run_adb_shell_process()` avec `adb_command`/`encoding`/`errors` optionnels, puis migre `core/clone/detection` dessus afin de garder le scan clone dans son owner tout en centralisant l'execution ADB.
...
- un sous-lot TikTok suivant transforme aussi `flows/publish.py` en vrai package `flows/publish/` et split les selectors entre `creation_entry.py`, `media_picker.py`, `editor.py`, `composer.py` et `progress.py`, tout en gardant `PUBLISH_SELECTORS` comme facade publique de compatibilite.
- un sous-lot TikTok suivant aligne ensuite les services/runtime `publish` sur les owners directs `ui/selectors/flows/publish/*`, afin de reserver `PUBLISH_SELECTORS` a la compatibilite plutot qu'aux imports internes.
- un sous-lot suivant du lot 4 clarifie aussi `taktik/core/compat` : le framework selectors/versioning et tracing compat vit maintenant sous `taktik/core/compat/selectors/**`, les bridges internes et `clone/selectors/patcher.py` importent ces owners directs, et les anciens shims top-level `compat/selector_registry.py`, `compat/selector_tracer.py`, `compat/setup.py` ont ete retires.
...
- un sous-lot suivant etend cette frontiere aux variantes TikTok utilisees par le publish/runtime : `package_resolver.py` ne depend plus de `tiktok/core/manager.py` pour connaitre les packages installables.
- un sous-lot suivant poursuit le palier publish/session lifecycle TikTok : le redemarrage d'app package est porte par `services/runtime/app_control.py`, et `upload_workflow.py` garde seulement le sequencage metier du publish.
...
- un sous-lot suivant du lot 5 clarifie `taktik/core/app/email/gmail` : `workflows/account.py` ne depend plus directement de `bridges.common.ipc`, expose un notifier injecte optionnel, et les bridges/workflows appelants (`bridges/gmail`, `bridges/youtube`, TikTok signup) lui passent maintenant leur IPC existant depuis l'exterieur.
...
- un sous-lot suivant restructure `taktik/core/app/email/gmail` : `workflows/` porte le workflow compte/OTP Gmail, `ui/` porte les selectors Gmail, et il n'y a plus de package racine `taktik/core/email`.
...
| `taktik/core/compat/__init__.py` | Facade publique du framework compat. | Compat selectors publique. | Tests et imports package-level. | Re-export vers `compat/selectors/**`. | Garder comme facade package-level ; ne pas recreer de shims top-level. |
