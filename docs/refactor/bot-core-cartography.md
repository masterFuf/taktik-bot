...
3. Lot 3 - managers runtime : `instagram/core/manager.py` et `tiktok/core/manager.py` pointent maintenant vers `shared/platform/social_media_base.py` et `shared/device/manager.py` comme `threads/core/manager.py`, au lieu de repasser par les shims Instagram. Prochain pas naturel : auditer les autres imports runtime encore branchés sur `social_media/instagram/actions/core/device` quand ils n'ajoutent aucun comportement plateforme.
