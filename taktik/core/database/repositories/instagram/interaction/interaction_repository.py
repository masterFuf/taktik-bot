...
class InteractionRepository(BaseRepository):
    ...
    def get_session_stats(self, session_id: int) -> Dict[str, int]:
        ...
        `profiles_engaged` counts the distinct profiles that received a gesture (like, follow,
        comment, story watch, like or reaction); an unfollow or a bare visit engages nobody.
        """
        row = self.query_one_orm_first(
            """
            SELECT
                ...
                COUNT(DISTINCT CASE WHEN interaction_type IN
                    ('LIKE', 'FOLLOW', 'COMMENT', 'STORY_WATCH', 'STORY_LIKE', 'STORY_REACTION')
                    THEN profile_id END) as profiles_engaged
            FROM interactions
        ...
        return {
            ...
            'profiles_engaged': row.get('profiles_engaged') or 0,
        }
