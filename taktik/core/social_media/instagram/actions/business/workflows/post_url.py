...
class PostUrlBusiness(BaseBusinessAction):
    ...
    def interact_with_post_likers(self, post_url: str, config: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        self.current_max_interactions = effective_config.get('max_interactions_per_session', effective_config.get('max_interactions', 20))
        self.logger.info(f"Max interactions configured for extraction: {self.current_max_interactions}")
        ...
        try:
            self.logger.info(f"Starting Post URL workflow: {post_url}")
            ...
            post_metadata = self._extract_post_metadata_from_url(post_url)
            if not post_metadata or not post_metadata.get('author_username'):
                self.logger.error("Failed to extract post metadata")
                stats['errors'] += 1
                return stats
            ...
            if not validation_result['valid']:
                ...
                if validation_result.get('adjusted_max'):
                    ...
                    self.current_max_interactions = validation_result['adjusted_max']
                    self.logger.info(f"✅ Adjusted max interactions to {validation_result['adjusted_max']}")
            
            author_username = post_metadata['author_username']
            self.logger.info(f"Post from @{author_username} identified")
            
            if not self.nav_actions.navigate_to_profile(author_username):
                self.logger.error(f"Failed to navigate to @{author_username}")
                stats['errors'] += 1
                return stats
            ...
            likers = likers[:max_interactions]
            self.logger.info(f"{len(likers)} users to process (max: {max_interactions})")
            ...
            for i, username in enumerate(likers, 1):
                ...
                if interaction_result:
                    ...
                    self.stats_manager.increment('comments', interaction_result.get('comments', 0))
