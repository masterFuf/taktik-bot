...
- Instagram automation config normalization now lives under `social_media/instagram/workflows/core/config_builder.py`; `desktop_bridge.py` delegates to it so future Agent handlers can reuse the same bridge-compatible payload mapping without owning bridge startup/runtime.
