...
- Les primitives ADB shell partagees vivent sous `taktik/core/shared/device/adb.py`. `shared/input/taktik_keyboard.py` peut les re-exporter pour compatibilite, mais le nouveau code device/runtime doit importer l'owner direct plutot qu'un module clavier.
- Utiliser `run_adb_shell_process()` depuis `shared/device/adb.py` quand un appelant a besoin du `returncode`, de `stderr` ou d'options subprocess ; garder `run_adb_shell()` pour les commandes qui consomment seulement stdout.
...
- Dans `taktik/core/agent/**`, utiliser les owners internes directs : `kernel/contracts.py` pour les dataclasses de plan/event, `kernel/ports.py` pour les interfaces injectees, `kernel/errors.py` pour les erreurs runtime structurees, `kernel/` pour contexte/registry/executor/runtime, `io/` pour manifest/plan/events, `decision/` pour la decision IA, `scenarios/` pour les autopilots legacy. La racine du package doit rester une facade publique via `__init__.py`, pas une liste de modules plats.
- Avant d'executer un `AgentPlan` multi-step depuis `core/agent`, pre-valider la couverture des handlers du `WorkflowRegistry`. Un plan incomplet doit echouer avant tout event ou effet de bord Android, pas au milieu d'une execution partielle.
- Les handlers reels branchables dans `WorkflowRegistry` vivent chez l'owner plateforme du workflow (`social_media/<platform>/workflows/**/agent_handler.py`). Ils adaptent un `WorkflowInvocation` vers le workflow existant, mais ne doivent pas ouvrir de connexion device, importer un bridge ou posseder le planning premium.
- Si plusieurs handlers Agent d'une meme famille plateforme partagent seulement des primitives d'adaptation (merge payload, coercition de parametres, callbacks notifier), les factoriser sous un owner interne de workflow nomme, par exemple `social_media/tiktok/actions/business/workflows/_internal/agent_runtime.py`. Ne pas recreer un `utils`/`helpers` global ni deplacer ces primitives dans `core/shared`.
...
- Si plusieurs workflows d'une meme plateforme partagent le mecanisme d'injection notifier, placer le support runtime sous un owner nomme de la plateforme. Exemple TikTok : `social_media/tiktok/workflows/runtime/notifier.py`, pas un helper racine ni un import bridge.
...
- `taktik/core/app/ai/**` est l'owner des integrations IA reutilisables par le Bot. Utiliser `app/ai/providers/**` pour les providers runtime comme OpenRouter et `app/ai/comments/**` pour l'IA commentaire/persona. Ne pas recreer de shim IA sous `bridges/common`.
...
- `taktik/core/clone/**` est l'owner transversal des variantes Android par package : detection de clones, packages officiels/alternatifs, package actif, proxy device clone-aware et patch de selectors par package. Ne pas y ajouter de logique metier Instagram/TikTok qui appartient a `social_media/<platform>`.
...
- Quand `bridges/<platform>/engagement/runtime/**` contient plusieurs familles de flows, regrouper par flow (`dm/**`, `cold_dm/**`, `smart_comment/**`, etc.) au lieu d'empiler des fichiers prefixes a plat.
- Quand un bridge de scraping a besoin de support local (emitters JSON, persistence de session/profil, runner workflow, signal handling), le ranger sous `bridges/<platform>/scraping/runtime/**` plutot que de gonfler l'entrypoint `scraping/<flow>.py`.
...
- Les entrypoints Electron de compat diagnostics vivent sous `bridges/compat/diagnostics/entrypoints/**`. Ne pas ajouter de nouveaux fichiers plats `action_test.py`, `workflow_test.py`, `selector_test.py`, `compat.py` ou `tiktok_action_test.py` a la racine `diagnostics/`.
...
- Sous `bridges/compat/diagnostics/runtime/workflow_test/**`, classer par responsabilite : `config/**` pour catalogues/requetes, `contracts/**` pour dataclasses/resultats partages, `execution/**` pour orchestration lifecycle/session/dispatch, `observability/**` pour etat/hooks de trace, `reporting/**` pour rapports, et `platforms/**` pour les branches Instagram/TikTok. Ne pas remettre des fichiers plats `catalog.py`, `request.py`, `dispatcher.py`, `session.py`, `report.py` ou `runners.py` a la racine workflow-test.
...
- Le Lab Cartography doit rester representatif de la production : les actions manuelles lancees en mode `lab` doivent reutiliser une session/process connecte quand c'est possible (`action_session_bridge`) au lieu de reconnecter le device, reconstruire le bundle et redetecter la langue a chaque clic. Le bridge single-shot reste acceptable pour un debug explicite hors session.
- Piege de dev : la session action-test persistante importe le code des actions/selecteurs une seule fois au demarrage. Apres avoir modifie une action ou un selector bot, redemarrer la session (bouton "Redemarrer session" du Lab, ou changement device/plateforme) sinon le Lab tourne sur l'ancien code en cache. Verifier l'ordre des selecteurs dans les logs avant de conclure qu'un fix ne marche pas.
- Les optimisations de performance du Lab Cartography qui concernent les selectors, waits ou changements d'ecran doivent viser l'owner production/shared quand elles modifient le comportement mesure. Une optimisation Lab-only est acceptable seulement si elle est purement diagnostique et documentee comme telle.
- Les tests Cartography Lab doivent reproduire la logique runtime de production quand elle influence les selectors. Exemple : reutiliser `detect_and_optimize()` pour filtrer les selectors de la mauvaise langue avant de mesurer la sante selector ; ne pas tester une matrice plus large que la prod sauf mode/audit explicitement documente.
- Les runs Cartography Lab doivent separer le resultat technique de l'action et la validation fonctionnelle d'ecran. Si une action a un `expectedScreenAfter`, le report doit exposer `transition.ok` et l'analyse doit signaler les faux succes au lieu de les masquer.
- Couverture obligatoire du Lab (regle de Kevin) : toute fonctionnalite bot qui produit une interaction ou une lecture a l'ecran (clic/tap, recuperation de texte/bio/commentaire, OCR, expansion « more »/« plus », navigation, scan, accept/ignore/reply...) DOIT etre exposee comme `@action` testable unitairement dans le Cartography Lab. Avant de cloturer un lot qui ajoute OU modifie ce type de capacite : verifier si l'action existe deja dans `bridges/compat/diagnostics/actions/<platform>/**` ; si elle manque, la cabler en reutilisant EXACTEMENT la meme fonction atomique/workflow de production (construire le workflow/mixin reel sur le device chaud, jamais un chemin ni un atomique Lab-only — cf. la regle "appeler exactement le chemin prod"). L'objectif est que Kevin puisse tester chaque point separement depuis le Lab. Cote front, mirorer l'action dans le catalogue + `cartography.json` (cf. `front/AGENTS.md`).
...
- Quand un bridge compat diagnostics expose des actions manuelles par plateforme, classer les catalogues sous `bridges/compat/diagnostics/actions/<platform>/**`; l'entrypoint compat doit seulement charger le registre et appeler le runner.
- Quand un bridge compat diagnostics a des hooks d'observabilite specifiques a une plateforme, les isoler sous `bridges/compat/diagnostics/runtime/*_<platform>.py` ou un sous-owner plateforme explicite ; la facade runtime publique doit rester mince et conserver le contrat stdout JSON.
...
- Quand un dispatcher `bridges/<platform>/workflows/dispatcher.py` route plusieurs workflows sans sous-famille existante, ses runners locaux peuvent vivre sous `bridges/<platform>/workflows/runtime/**`. Le dispatcher doit rester limite a lecture config, routing, erreurs et cleanup.
- Sous `bridges/common/**`, classer par capacite de bridge (`device`, `input`, `parsing`, `runtime`, `persistence`, etc.) au lieu d'ajouter de nouveaux modules plats. Exemples : la connexion device et le lifecycle app vivent sous `bridges/common/device/**`, le catalogue package/activity sous `bridges/common/device/apps.py`, le stdout JSON/bootstrap/signal handling sous `bridges/common/runtime/**`, la saisie clavier sous `bridges/common/input/keyboard.py`, les parseurs de payload/texte sous `bridges/common/parsing/**`, et les facades DB de bridge sous `bridges/common/persistence/**`.
...
- Sous `bridges/common/device/**`, garder les probes reseau dans `network_probe.py`, les strategies de reset dans `network_reset.py`, et `network.py` comme facade/orchestrateur IPC de compatibilite.
...
- Les repositories workflow TikTok doivent aussi vivre sous un owner de domaine explicite. Exemple : l'adapter DB du workflow Followers vit sous `repositories/tiktok/followers/followers_repository.py` et expose `TikTokFollowersRepository` depuis ce sous-package, pas depuis un module plat `followers_repository.py` ni depuis la racine `repositories/tiktok/__init__.py`.
...
- Tout workflow/runtime Instagram/TikTok qui utilise des selectors dependants de la langue (`L(...)`, ou n'importe quelle signature texte/`content-desc`) DOIT detecter la langue de l'app au demarrage via `detect_and_optimize(device)` (`ui/language.py`), comme le font `workflows/core/runtime_setup.py` et `change_language`. L'appeler une fois, sur un ecran texte stable, avant tout selector localise (best-effort / non bloquant). Sinon les signatures purement textuelles sans `resource-id` (ex. en-tete groupe "Demandes de suivi" / "Follow requests") echouent silencieusement sur un device dans une autre langue. Ne pas "corriger" ce probleme en passant un selector en `L_all` au cas par cas : la detection de langue est le mecanisme unique.
...
- Pour compat diagnostics : lancer `python scripts/audit_diagnostics_runtime_layout.py` apres tout changement sous `bridges/compat/diagnostics/**` afin de bloquer les retours de modules plats dans `diagnostics/`, `runtime/` ou `workflow_test/`.
