...
| P1 SQL direct handlers | 87% | `targetSearch`, scraping, DB commun, scheduler, analytics/profiling, sessions/interactions/stats, accounts/profiles/following, Smart Comment DB, Gmail DB, relationship graph DB, TikTok DB, database explorer, DB export/tutorial, storage media cleanup, configuration locale device/network et plusieurs routes TikTok ont ete nettoyes. Cote Electron, Cold DM ne remappe plus les profils scraping dans le handler, Target Search n'ecrit plus l'export CSV depuis l'IPC, `storage:*` delegue usage disque + cleanup DB a un service owner, `device-groups`/`network-*` passent par des services app, `analytics.ts` delegue a `AnalyticsProfilingService`, `sessions.ts` delegue a `SessionDatabaseService`, `accounts.ts` delegue a `InstagramAccountDatabaseService`, `smart-comment.ts` delegue a `SmartCommentDatabaseService`, `gmail.ts` delegue a `GmailAccountDatabaseService`, `graph.ts` delegue a `RelationshipGraphDatabaseService`, `tiktok.ts` delegue a `TikTokDatabaseService`, `explorer.ts` delegue a `DatabaseExplorerService` et `export.ts` delegue a `DatabaseExportService`/`TutorialProgressService`. Reste des zones ponctuelles comme AI media/diagnostics et exceptions documentees. |
